...
impl MarketState {
    ...
    pub fn on_price_goes_up(&mut self, new_price: U256) {
        // Protect against zero prices and overflow
        if self.price.is_zero() || new_price.is_zero() {
            self.price = new_price;
            return;
        }

        // Use saturating arithmetic to prevent overflow
        let numerator = self.short_liquidity.saturating_mul(new_price);
        let quotient = if self.price > U256::zero() {
            numerator / self.price
        } else {
            U256::zero()
        };
        let delta = quotient.saturating_sub(self.short_liquidity);
        let delta = self.short_liquidity.min(delta);

        self.short_liquidity = self.short_liquidity.saturating_sub(delta);
        self.long_liquidity = self.long_liquidity.saturating_add(delta);
        self.price = new_price;
    }
    ...
    pub fn on_price_goes_down(&mut self, new_price: U256) {
        ...
        // Use saturating arithmetic to prevent overflow
        let numerator = self.long_liquidity.saturating_mul(self.price);
        let quotient = if new_price > U256::zero() {
            numerator / new_price
        } else {
            U256::zero()
        };
        let delta = quotient.saturating_sub(self.long_liquidity);

        self.long_liquidity = self.long_liquidity.saturating_sub(delta);
        self.short_liquidity = self.short_liquidity.saturating_add(delta);
        self.price = new_price;
    }
    ...
    pub fn on_deposit(&mut self, side: Side, amount: U256) -> U256 {
        ...
        let delta_token_supply = if old_token_supply.is_zero() || old_liquidity.is_zero() {
            amount
        } else {
