...
#[then(expr = "{account} transfers {amount} {token_kind} to {account}")]
fn transfer(
    world: &mut CasperShortsWorld,
    sender: Account,
    amount: Amount,
...
#[then(expr = "{account} cannot transfer {amount} {token_kind} to {account}")]
fn cannot_transfer(
    world: &mut CasperShortsWorld,
    sender: Account,
    amount: Amount,
    ...
    let result = world.try_transfer(token, sender, amount.value(), receiver);
