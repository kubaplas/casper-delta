import init, { Address, OdraWasmClient, U256, TransactionStatus, setGas, CsprClickCallbacks, } from "casper-delta-wasm-client";
// Import the generated clients from casper-delta
import { MarketWasmClient, FaucetableWcsprWasmClient, PositionTokenWasmClient, } from "casper-delta-wasm-client";
// ---------- Types ----------
let client;
let market;
let wcspr;
...
function showTransactionPopup(description) {
...
async function onTransactionSuccessFromCsprClick(data) {
...
async function onTransactionFailureFromCsprClick(data) {
...
let marketAllowanceValue = null;
// Position closing state
let currentLongClosePercentage = 0;
let currentShortClosePercentage = 0;
let currentLongCloseAmount = null;
let currentShortCloseAmount = null;
// ---------- DOM Elements ----------
...
const addressDropdownMenu = document.getElementById("address-dropdown-menu");
const switchAccountBtn = document.getElementById("switch-account-btn");
// Market state elements
const currentPriceSpan = document.getElementById("current-price");
const totalMarketValueSpan = document.getElementById("total-market-value");
const totalMarketValueLoader = document.getElementById("total-market-value-loader");
const longLiquiditySpan = document.getElementById("long-liquidity");
const shortLiquiditySpan = document.getElementById("short-liquidity");
const priceLoader = document.getElementById("price-loader");
const longLiquidityLoader = document.getElementById("long-liquidity-loader");
const shortLiquidityLoader = document.getElementById("short-liquidity-loader");
// Balance elements
const wcsprBalanceSpan = document.getElementById("wcspr-balance");
const wcsprBalanceLoader = document.getElementById("wcspr-balance-loader");
// Position value display elements
const longPositionValueDisplay = document.getElementById("long-position-value-display");
const shortPositionValueDisplay = document.getElementById("short-position-value-display");
const longPositionValueLoader = document.getElementById("long-position-value-loader");
const shortPositionValueLoader = document.getElementById("short-position-value-loader");
// Position-specific balance elements
const wcsprBalanceLong = document.getElementById("wcspr-balance-long");
const wcsprBalanceShort = document.getElementById("wcspr-balance-short");
const wcsprBalanceLongLoader = document.getElementById("wcspr-balance-long-loader");
const wcsprBalanceShortLoader = document.getElementById("wcspr-balance-short-loader");
// Position value elements
const totalPositionValueSpan = document.getElementById("total-position-value");
const longPositionLoader = document.getElementById("long-position-loader");
const shortPositionLoader = document.getElementById("short-position-loader");
const totalPositionLoader = document.getElementById("total-position-loader");
// Trading inputs and buttons
const longOpenAmountInput = document.getElementById("long-open-amount");
const shortOpenAmountInput = document.getElementById("short-open-amount");
// Position closing elements
const longCloseAmountInput = document.getElementById("long-close-amount-input");
const shortCloseAmountInput = document.getElementById("short-close-amount-input");
const longCloseAmountDisplay = document.getElementById("long-close-amount-display");
const shortCloseAmountDisplay = document.getElementById("short-close-amount-display");
const longClosePercentage = document.getElementById("long-close-percentage");
const shortClosePercentage = document.getElementById("short-close-percentage");
// Percentage buttons
const longClose25Btn = document.getElementById("long-close-25");
const longClose50Btn = document.getElementById("long-close-50");
const longClose75Btn = document.getElementById("long-close-75");
const longClose100Btn = document.getElementById("long-close-100");
const shortClose25Btn = document.getElementById("short-close-25");
const shortClose50Btn = document.getElementById("short-close-50");
const shortClose75Btn = document.getElementById("short-close-75");
const shortClose100Btn = document.getElementById("short-close-100");
const depositLongBtn = document.getElementById("deposit-long-btn");
const withdrawLongBtn = document.getElementById("withdraw-long-btn");
const depositShortBtn = document.getElementById("deposit-short-btn");
const withdrawShortBtn = document.getElementById("withdraw-short-btn");
// Action buttons
const updatePriceBtn = document.getElementById("update-price-btn");
const refreshBtn = document.getElementById("refresh-btn");
const faucetBtn = document.getElementById("faucet-btn");
const approveMarketBtn = document.getElementById("approve-market-btn");
// Approve input and display
const approveAmountInput = document.getElementById("approve-amount");
const marketAllowanceSpan = document.getElementById("market-allowance");
const marketAllowanceLoader = document.getElementById("market-allowance-loader");
// Notification elements
const txSection = document.getElementById("tx-section");
const txLinkAnchor = document.getElementById("tx-link");
const errorSection = document.getElementById("error-section");
const errorText = document.getElementById("error-text");
// Error modal elements
const errorModalOverlay = document.getElementById("error-modal-overlay");
const errorModalMessage = document.getElementById("error-modal-message");
const errorModalPanel = document.getElementById("error-modal-panel");
const errorModalClose = document.getElementById("error-modal-close");
const marketStatusSpan = document.getElementById("market-status");
// Portfolio balances (long/short token counts in header widgets)
const longTokenBalancePortfolio = document.getElementById("long-token-balance-portfolio");
const shortTokenBalancePortfolio = document.getElementById("short-token-balance-portfolio");
// Transaction progress elements
const txProgressOverlay = document.getElementById("tx-progress-overlay");
const txProgressSpinner = document.getElementById("tx-progress-spinner");
const txProgressStatus = document.getElementById("tx-progress-status");
const txProgressHash = document.getElementById("tx-progress-hash");
const txProgressBar = document.getElementById("tx-progress-bar");
const txProgressTime = document.getElementById("tx-progress-time");
const txProgressPanel = document.getElementById("tx-progress-panel");
// Trading info elements
const tradingInfoSection = document.getElementById("trading-info-section");
const showTradingInfoSection = document.getElementById("show-trading-info-section");
const closeTradingInfoBtn = document.getElementById("close-trading-info");
const showTradingInfoBtn = document.getElementById("show-trading-info");
// ---------- Utility Functions ----------
function lockScroll() {
...
function formatNumber(value, decimals = TOKEN_DECIMALS) {
    try {
        let formatter;
...
function formatPrice(value, decimals = TOKEN_DECIMALS) {
    try {
...
function formatAllowance(value, decimals = TOKEN_DECIMALS) {
    try {
...
function formatDollarPrice(value, decimals = TOKEN_DECIMALS) {
    try {
...
function parseAmountFromNumber(value) {
...
function showTransactionHashInProgress(hash) {
    const url = `${EXPLORER_BASE.replace(/\/+$/, "")}/transaction/${hash}`;
...
function updateLongCloseAmount(percentage) {
...
function updateShortCloseAmount(percentage) {
    if (!consolidatedData || !consolidatedData.addressMarketState)
        return;
...
async function onConnect() {
    ...
    if (marketStatusSpan)
        marketStatusSpan.textContent = "Connected";
...
function updateUIFromConsolidatedData() {
    ...
    wcsprBalanceLong.onclick = () => {
        try {
            longOpenAmountInput.value = formatNumber(availableWcspr);
        }
        catch { }
    };
    wcsprBalanceShort.onclick = () => {
        try {
            shortOpenAmountInput.value = formatNumber(availableWcspr);
        }
        catch { }
    };
...
function transactionPreCheck(errorMessage) {
    if (!connected || !address) {
...
function onTransactionSentFailure(error, message) {
...
function showErrorWithTransaction(msg, hash) {
    if (errorModalMessage && errorModalOverlay) {
...
async function run() {
    try {
        ...
        if (marketStatusSpan)
            marketStatusSpan.textContent = "Ready";
    }
    catch (err) {
        ...
        if (marketStatusSpan)
            marketStatusSpan.textContent = "Error";
    }
}
